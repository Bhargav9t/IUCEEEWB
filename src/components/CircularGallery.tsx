...
interface MediaProps {
  ...
  borderRadius?: number;
  font?: string;
  originalItem: { image: string; text: string };
}
...
class Media {
  ...
  constructor({
    ...
    this.createShader();
    this.createMesh();
    // this.createTitle(); // Disabled to optimize performance and remove picture names
    this.onResize();
  }
...
interface AppConfig {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  onCardClick?: (item: { image: string; text: string }) => void;
}
...
class App {
  ...
  downX: number = 0;
  downY: number = 0;
  onCardClickCallback?: (item: { image: string; text: string }) => void;
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.downX = 'touches' in e ? e.touches[0].clientX : e.clientX;
    this.downY = 'touches' in e ? e.touches[0].clientY : e.clientY;
  }
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    const timeDiff = Date.now() - this.downTime;
    const upX = e && 'changedTouches' in e ? e.changedTouches[0].clientX : (e as MouseEvent)?.clientX;
    const upY = e && 'changedTouches' in e ? e.changedTouches[0].clientY : (e as MouseEvent)?.clientY;

    if (upX !== undefined && upY !== undefined && timeDiff < 250) {
      const dx = upX - this.downX;
      const dy = upY - this.downY;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist < 10) {
        this.onClick(upX, upY);
      }
    }
  }
  ...
  onClick(clientX: number, clientY: number) {
    ...
    if (hits.length > 0) {
      ...
      if (hitMedia && this.onCardClickCallback) {
        this.onCardClickCallback(hitMedia.originalItem);
      }
    }
  }
...
interface CircularGalleryProps {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  onImageClick?: (item: { image: string; text: string }) => void;
}
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onImageClick]);
  return (
    <div
